...
### ⚠️ 绝对禁止：清除 CPU-GPU 共享内存

DGX Spark 的 121GB 统一内存在 CPU 和 GPU 之间共享。**绝对不能擅自清除共享内存段**（如 `ipcrm`、手动清理 `/dev/shm`、或任何释放共享内存的操作）。

- **后果**：Tailscale 连接会**永久丢失**，机器将无法远程访问。
- **原因**：Tailscale 依赖共享内存中的某些运行时状态，清除后无法恢复，只能物理接触机器重新配置。
- **规则**：任何涉及内存清理的操作必须先确认不影响 Tailscale。不确定就别动。
