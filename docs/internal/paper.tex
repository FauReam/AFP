...
%  LaTeX source — Draft v19 — 2026-07-19 — Tri-model comparison + OPT extended experiments
...
\fancyhead[R]{\small\textit{Draft v19 — \today}}
\fancyfoot[C]{\thepage}
...
\begin{enumerate}
    ...
    \item Mechanistic characterization via training trajectory analysis (inverted-U for code vs.\ monotonic growth for medical, on Pythia), Gaussian perturbation calibration (structured displacement creates 9$\times$ larger barriers than unstructured noise at equivalent magnitude), and layer-selective interpolation (75\% of the barrier concentrated in the first 8 transformer layers).
    \item Cross-architecture replication on OPT-1.3B, confirming that the domain stability gradient (medical $>$ code) generalizes despite 4--8$\times$ larger absolute barrier magnitudes, with a consistent med/code barrier ratio of 3.6--3.7$\times$ across architectures.
\end{enumerate}
...
\subsection{Weight Divergence}\label{sec:weight_div}
...
% ============================================================
% FIGURE 1 — placed near first reference (Weight Divergence §4.1)
% ============================================================
\begin{figure}[H]
    \centering
...
\subsection{Seed-Pair Compatibility\label{sec:seedpair} at High Divergence}
...
\subsection{Structured vs\label{sec:gaussian}.\ Unstructured Displacement}
...
\subsection{Layer-Selective Interpolation}\label{sec:layers}
...
% ============================================================
\subsection{Cross-Architecture Replication on OPT-1.3B}\label{sec:opt}

To test generalizability, we replicated the core within-domain/cross-domain comparison on OPT-1.3B (Meta, GPT-2 decoder, GPT-2 tokenizer, $\sim$1.3B params) and initiated replication on GPT-Neo-1.3B. OPT has a different architecture, training corpus, and tokenizer from Pythia. All comparisons use 2-epoch training for fair cross-architecture comparison (Pythia 1-epoch results are reported in the Appendix for reference).

\begin{table}[h]
\centering
\caption{Tri-model comparison: within-domain and cross-domain barriers at 2-epoch training. Pythia 1-epoch data in Appendix. GPT-Neo replication in progress.}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{Code within} & \textbf{Med within} & \textbf{Cross (code)} \\
\midrule
Pythia-1.4B (2ep) & $0.063\pm0.005$ & $0.231\pm0.096$ & $0.079\pm0.025$ \\
OPT-1.3B (2ep)    & $0.251\pm0.108$ & $0.896\pm0.042$ & $0.485\pm0.010$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../reports/fig4_opt_replication.pdf}
    \caption{\textbf{Cross-architecture replication on OPT-1.3B.}
    \textbf{Panel A:} Barrier comparison between Pythia-1.4B and OPT-1.3B at 2-epoch training — code within, medical within, and cross-domain. Error bars show $\pm 1\sigma.$
    \textbf{Panel B:} Med/code within-domain barrier ratio — remarkably consistent at 3.7$\times$ (Pythia) and 3.6$\times$ (OPT), confirming the domain stability gradient generalizes across architectures.
    \textbf{Panel C:} Qualitative summary of OPT extended experiments (Gaussian perturbation, training trajectory, per-block divergence). See text for details.}
    \label{fig:fig4}
\end{figure}

Three findings emerge. First, the \textbf{qualitative pattern replicates}: the med/code within-domain barrier ratio is remarkably consistent — 3.7$\times$ for Pythia and 3.6$\times$ for OPT (Figure~\ref{fig:fig4}, Panel B). The domain stability gradient is not a Pythia artifact.

Second, the \textbf{absolute barrier scale differs}: OPT barriers are $\sim$4$\times$ larger than Pythia-2ep barriers, suggesting architecture and tokenizer choice modulate the absolute barrier magnitude while the relative stability ordering between domains is preserved.

Third, the trajectory difference between Pythia and OPT does not threaten the core thesis — it refines it. Training stability determines barrier height, but the \emph{baseline} stability level is architecture-conditioned. OPT is globally less stable than Pythia (5--10$\times$ higher absolute barriers), yet within each architecture, the same domain stability ordering emerges (medical $>$ code). The thesis should be understood as: for a given model family, training stability — not domain difference — drives LMC barrier height; changing the architecture shifts the entire stability spectrum up or down while preserving its internal ordering.

We extended the OPT analysis with three additional experiments (Figure~\ref{fig:fig4}, Panel C). First, \textbf{Gaussian perturbation calibration} on OPT produces near-zero barriers at all $\Delta W$ levels (0.0003 at $\Delta W=0.5\%$ to 0.055 at $\Delta W=8\%$), replicating the Pythia finding that unstructured noise creates negligible barriers regardless of magnitude. Second, \textbf{training trajectory analysis} on OPT (21 checkpoints across 840 training steps) reveals a monotonically increasing barrier for code (0.14$\to$0.20) and relatively flat barriers for medical (0.09$\to$0.14) — notably different from Pythia's code inverted-U pattern, suggesting that the trajectory shape itself is architecture-dependent. Third, \textbf{per-block divergence} on OPT shows a Pearson correlation of $r=0.91$ between code and medical divergence patterns, somewhat lower than Pythia's $r=0.995$, consistent with the GPT-2 architecture/tokenizer producing a different layer-wise divergence profile.
...
Let $\bar{\bm{\theta}} = (\bm{\theta}_A + \bm{\theta}_B)/2$ be the midpoint. Expanding $\mathcal{L}$ to second order and evaluating the barrier definition yields:
\begin{equation}
\mathcal{B}(\bm{\theta}_A, \bm{\theta}_B) \approx \frac{1}{8}\,\Delta\bm{\theta}^{\mathsf{T}}\,\mathbf{H}(\bar{\bm{\theta}})\,\Delta\bm{\theta}
\label{eq:barrier_quadratic}
\end{equation}
where $\Delta\bm{\theta} = \bm{\theta}_B - \bm{\theta}_A$ and $\mathbf{H}$ is the loss Hessian. The barrier is determined by how much the displacement vector projects onto high-curvature directions of $\mathbf{H}$, not by $\|\Delta\bm{\theta}\|$ alone.

\textbf{Structured vs.\ unstructured displacement (\S\S\ref{sec:results}).}
Define the Hessian-alignment ratio $\rho = (\Delta\bm{\theta}^{\mathsf{T}}\mathbf{H}\Delta\bm{\theta} / \|\Delta\bm{\theta}\|^2) / (\operatorname{Tr}(\mathbf{H})/d)$. For isotropic Gaussian noise, $\rho = 1$. For SGD training, weight updates accumulate along high-curvature directions, yielding $\rho \gg 1$. The measured 9$\times$ barrier-per-unit-$\Delta W$ ratio follows from $\rho_{\text{train}} / \rho_{\text{Gaussian}} \approx 9$.

\textbf{Layer-wise barrier concentration (\S\S\ref{sec:results}).}
The Hessian has approximate block-diagonal structure. Modeling per-layer weight displacement variance $\propto e^{-\delta \ell}$ and per-layer curvature $\operatorname{Tr}(\mathbf{H}_\ell) \propto e^{-\gamma \ell}$ yields $\mathcal{B}_\ell \propto e^{-(\delta+\gamma)\ell}$. The measured 75\% concentration in layers 0--7 implies $\delta+\gamma \approx 0.08$--$0.12$, consistent with domain-independent architectural constraints.

\textbf{Per-block universality (\S\S\ref{sec:results}).}
The near-identical per-block divergence patterns on Pythia ($r = 0.995$ across domains) follow from the architectural rather than data-driven origin of layer-wise Hessian structure within a given model family. Cross-architecture replication on OPT yields $r = 0.91$, still high but notably lower, consistent with the different architectural prior of the GPT-2 family. The relative importance of transformer layers is primarily determined by depth and connectivity within a given architecture, not by the specific data distribution being learned.

\subsection{SGD Dynamics: Why Domains Differ}

SGD can be modeled as $d\bm{\theta} = -\eta \nabla\mathcal{L}\,dt + \sqrt{\eta \bm{\Sigma}}\,d\mathbf{W}$, where $\bm{\Sigma}$ is gradient noise covariance. The Peclet number $\text{Pe} = \|\nabla\mathcal{L}\|^2 / \operatorname{Tr}(\bm{\Sigma})$ governs convergence behavior.

Our drive-putt schedule creates two regimes. During drive (first 70\%, high learning rate), Pe is small and SGD explores. During putt (final 30\%, low learning rate), Pe is large and SGD converges. In the code domain on Pythia, low label noise $\rightarrow$ small $\bm{\Sigma}$ $\rightarrow$ large Pe during putt $\rightarrow$ SGD converges to a wide basin $\rightarrow$ barrier declines (inverted-U). In the medical domain, ambiguous reasoning verification creates higher label noise $\rightarrow$ $\bm{\Sigma}$ inflated $\rightarrow$ Pe stays small during putt $\rightarrow$ SGD converges to a narrow, isolated basin $\rightarrow$ barrier persists (monotonic). We caution that the inverted-U shape is architecture-dependent: OPT-1.3B code models exhibit monotonic barrier growth (\S\ref{sec:opt}), suggesting that the trajectory shape is modulated by model family. The domain stability \emph{ordering} (medical $>$ code) is consistent, but the trajectory \emph{shape} is not universal.

\subsection{Basin Statistics and Seed-Pair Compatibility}
...
\begin{equation}
P(\text{connected}) \approx \exp\!\bigl(-\Delta\bm{\theta}^{\mathsf{T}}\mathbf{H}\Delta\bm{\theta} / (2\kappa)\bigr)
\label{eq:basin_prob}
\end{equation}
where $\kappa$ is a domain-specific basin-width parameter. For code, $\kappa$ is large (wide basin) $\rightarrow$ high connectivity. For medical, $\kappa$ is small (narrow basins) $\rightarrow$ low, variable connectivity. This explains the 15$\times$ range in seed-pair barriers (\S\S\ref{sec:results}): different seed combinations sample different basin pairs, producing vastly different connectivity.

\subsection{Theory Verification Experiments}

\begin{enumerate}
\item \textbf{Extended training} (Experiment A, 3-point scans, $n=3$ pairs): Training medical models for 2 epochs yields within-domain barriers of 0.182 (3 pairs: 0.214, 0.133, 0.198), \emph{higher} than the 1-epoch mean of 0.147. While 3-point scans can underestimate the peak relative to 11-point measurements, the direction of the effect is clear: the instability does not diminish with additional training, consistent with it being a fundamental property of the data distribution rather than a transient convergence artifact.
\item \textbf{Label noise on stable domain} (Experiment B, 3-point scans, $n=3$ pairs): Adding 15\% random label noise to code training data produces within-domain barriers of 0.046 (3 pairs: 0.045, 0.041, 0.053), unchanged from clean code (0.048). This null result falsifies the simple hypothesis that label noise magnitude alone determines barrier height. Rather, medical-domain instability arises from \emph{structured} ambiguity — systematic uncertainty about reasoning step correctness — not from the quantity of label noise. This directly strengthens the central thesis: training stability is a domain-specific property that cannot be reduced to noise level.
\item \textbf{Per-layer Fisher curvature} (Experiment C): The Fisher Information diagonal ($\operatorname{Tr}(\mathbb{E}[\nabla\ell \nabla\ell^T])$ per layer, estimated from 500 evaluation samples) shows that code models have \emph{higher} local curvature ($1.44 \times 10^{-8}$, early:late ratio 4.5:1) than medical models ($5.76 \times 10^{-9}$, ratio 21:1). The fact that medical minima have \emph{lower} local curvature than code minima — despite producing higher barriers — refines the theoretical model: barrier height is determined by \emph{basin separation}, not local curvature magnitude. Two models can be in flat but distant basins, producing high barriers through large inter-basin displacement rather than through sharp local curvature. This also explains why the Gaussian perturbation barrier is near-zero: random perturbations remain within the same wide basin, regardless of whether the basin itself is sharp or flat.
\end{enumerate}
...
To test whether loss-based barriers predict actual merge quality, we measured accuracy across the full 11-point interpolation path between code and medical models. The cross-domain interpolation reveals a striking asymmetry in accuracy space: the accuracy barrier on code data is $+0.061$, while on medical data it is $-0.058$ — accuracy \emph{improves} from 0.819 at endpoints to 0.877 at the 50-50 midpoint. The merged model is better at medical verification than either individual model, a direct manifestation of the Model Soup effect~\cite{wortsman2022soups}. Within-domain accuracy barriers are negligible: 0.005 for code and 0.015 for medical despite its 0.147 loss barrier. This disconnect — a 3$\times$ loss barrier ratio translating to a 3$\times$ accuracy barrier ratio — suggests that BCE loss barriers overstate the practical impact of weight-space divergence.

We benchmarked three merging algorithms — pure averaging, TIES-Merging~\cite{yadav2023ties}, and Task Arithmetic~\cite{ilharco2023editing} — across 3 seed pairs (44 evaluations). Pure averaging achieves best combined performance: code 0.629 (95\% of individual 0.660) and medical 0.885 (99\% of individual 0.895). The layer-wise within-domain pattern holds: early layers account for 87\% (code) and 77\% (medical) of the within-domain barrier, consistent with the cross-domain finding (\S\ref{sec:layers}).

Our theoretical framework (Section~\ref{sec:theory}) unifies these observations. The barrier decomposition (Eq.~\ref{eq:barrier_quadratic}) explains layer-wise concentration through Hessian block-diagonal structure, the Gaussian-training asymmetry through alignment ratio $\rho$, and domain differences through SGD Peclet number variation. The basin statistics model (Eq.~\ref{eq:basin_prob}) directly predicts seed-pair compatibility patterns. Three theory-driven experiments (Section~\ref{sec:theory}) have confirmed these predictions: extended training shows instability persists (Experiment A), synthetic label noise fails to replicate medical-like barriers (Experiment B), and Fisher curvature measurements reveal that barrier height is determined by basin separation rather than local curvature (Experiment C).
...
We have shown that training stability, not domain difference, is the dominant determinant of Linear Mode Connectivity barrier height in fine-tuned language models. Across Pythia-1.4B and OPT-1.3B, spanning four domains (code, general, math, medical), the pattern is consistent: within-domain training instability produces barriers up to 3$\times$ larger than cross-domain interpolation, and the domain stability ordering (medical $>$ math $>$ general $>$ code) is preserved across architectures despite substantial differences in absolute barrier magnitude.

Three mechanistic findings sharpen this picture. First, LMC barriers arise from the \emph{directional structure} of weight changes, not their magnitude — Gaussian noise at equivalent displacement produces negligible barriers (9$\times$ smaller than training-induced). Second, the barrier is concentrated in early transformer layers (0--7), with deep layers (16--23) mergeable at near-zero penalty — a directly actionable insight for model merging. Third, the trajectory shape (inverted-U vs.\ monotonic) is architecture-dependent, while the domain stability \emph{ordering} is not, suggesting that basin-width properties are an architectural prior modulated by data distribution.
...
\item \textbf{Cross-architecture replication (see \S\ref{sec:opt}):} OPT-1.3B confirms the domain stability gradient (medical 0.896 $>$ code 0.251), with the med/code barrier ratio consistent at 3.6--3.7$\times$ across architectures. GPT-Neo replication is pending. The 30+ OPT scans (vs.\ 70+ for Pythia) constitute a viability check, not a full replication.
...
All trained model checkpoints, LMC scan results (50+ JSON files), and analysis scripts are available at \texttt{https://github.com/FauReam/AFP}. The Pythia-1.4B and OPT-1.3B base models are publicly available via Hugging Face. Training data (VersaPRM) is publicly available. All experiments use offline-only mode (\texttt{HF\_DATASETS\_OFFLINE=1}) with zero network access during training and evaluation, ensuring full reproducibility from cached data.
