...
%  LaTeX source — Draft v13 — 2026-07-14 — Theory + Figures + Running Exps
...
\fancyhead[R]{\small\textit{Draft v13 — \today}}
\fancyfoot[C]{\thepage}
...
\begin{enumerate}
\item \textbf{Extended putt phase} (Experiment A, running): Training medical models for 2 epochs with extended low-LR convergence should decrease within-domain barriers as SGD settles into wider basins.
\item \textbf{Label noise on stable domain} (Experiment B, running): Adding 15\% label noise to code training data should inflate $\bm{\Sigma}$, reducing Pe and producing medical-like (higher, monotonic) barrier patterns.
\item \textbf{Hessian eigenvalue measurement} (Experiment C, running): Medical final models should exhibit larger top Hessian eigenvalues than code models, directly validating Eq.~\ref{eq:barrier_quadratic} via $\lambda_{\max}^{\text{(med)}} > \lambda_{\max}^{\text{(code)}}$.
\end{enumerate}
