...
A practical implication: our layer-selective results suggest that the standard "interpolate everything" approach to model merging is suboptimal — practitioners should consider interpolating only deep layers (16-23) while preserving domain-specific early representations. For domains like medical where training trajectories are intrinsically noisy, merging different seeds of the same domain can be more challenging than merging across domains. To test whether barrier measurements predict actual merge quality, we performed a simple 50-50 weight average of code and medical models (standard divergence, seed 0) and evaluated accuracy on both test sets. The merged model retains 99% of the medical model's accuracy (0.877 vs. 0.888) but only 86% of the code model's accuracy (0.566 vs. 0.660), consistent with the asymmetry finding that medical-domain weight changes dominate the interpolation. The layer-selective barrier analysis (§4.8) suggests a mitigation: averaging only deep layers would preserve more code-specific performance. Systematic merge-quality evaluation across merging algorithms (TIES, Task Arithmetic) and downstream benchmarks is a natural next step.
...
- **Single model family.** Primary results are on Pythia-1.4B. A Pythia-160M baseline (ΔW ≈ 0.3%, barrier ≈ 0.001) is consistent with the ΔW→barrier trend at low divergence. We are currently replicating the core within-domain/cross-domain comparison on OPT-1.3B (Meta, GPT-2 architecture, ~1.3B params) to test generalizability across model families with different architectures, training corpora, and tokenizers.
