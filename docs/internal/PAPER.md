...
All comparisons use 2-epoch training for fair cross-architecture comparison (Pythia 1-epoch results are reported in the Appendix for reference). The **qualitative pattern replicates**: the med/code within-domain barrier ratio is remarkably consistent — 3.7× (Pythia) and 3.6× (OPT). The domain stability gradient is not a Pythia artifact.

The **absolute barrier scale differs**: OPT barriers are ~4× larger than Pythia-2ep barriers. This suggests that architecture and tokenizer choice modulate the absolute barrier magnitude, while the relative stability ordering between domains is preserved. Both models show code within < code cross, consistent with cross-domain models being farther apart in weight space than within-domain seed pairs. This suggests that code-trained OPT models are more similar to each other than to medical models — the tokenizer and architecture modulate both the absolute barrier scale and the relative positioning of domains in weight space. The core finding — medical training is less stable than code training — generalizes, but the detailed barrier structure is architecture-dependent.
