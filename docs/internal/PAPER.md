...
| Model | Code within | Medical within | Code↔Med cross | med/code ratio |
|-------|:---:|:---:|:---:|:---:|
| Pythia-1.4B (1ep) | 0.048 ± 0.000 | 0.147 ± 0.027 | 0.053 ± 0.011 | 3.1× |
| Pythia-1.4B (2ep) | 0.063 ± 0.005 | 0.231 ± 0.096 | 0.079 ± 0.025 | 3.7× |
| OPT-1.3B (2ep) | 0.251 ± 0.108 | 0.896 ± 0.042 | 0.485 ± 0.010 | 3.6× |

The **qualitative pattern replicates**: the med/code within-domain barrier ratio is remarkably consistent across all three conditions — 3.1× (Pythia 1ep), 3.7× (Pythia 2ep), and 3.6× (OPT 2ep). Training depth raises absolute barrier levels but does not alter the domain stability ordering. The domain stability gradient is not a Pythia artifact and is not an artifact of training depth.

However, the **absolute barrier scale differs**: OPT barriers are 5-6× larger than Pythia 1ep, and Pythia 2ep barriers are 1.3-1.6× larger than Pythia 1ep. Within/cross structure also varies: Pythia 1ep shows code within ≈ code cross (0.048 vs. 0.053), while Pythia 2ep (0.063 vs. 0.079) and OPT 2ep (0.251 vs. 0.485) both show code within < code cross. Extended training increases within-domain barriers relative to cross-domain barriers, but the medical > code ranking persists. This suggests that code-trained OPT models are more similar to each other than to medical models — the tokenizer and architecture modulate both the absolute barrier scale and the relative positioning of domains in weight space. The core finding — medical training is less stable than code training — generalizes, but the detailed barrier structure is architecture-dependent.

The trajectory difference between Pythia and OPT does not threaten the paper's core thesis — it refines it. Training stability determines barrier height, but the *baseline* stability level is architecture-conditioned. OPT is globally less stable than Pythia (5-10× higher absolute barriers), yet within each architecture, the same domain stability ordering emerges (medical > code). The thesis should be understood as: for a given model family, training stability — not domain difference — drives LMC barrier height; changing the architecture shifts the entire stability spectrum up or down while preserving its internal ordering. The Pythia 2-epoch trajectory experiment (currently running) will determine whether the inverted-U pattern persists with extended training or converges toward the OPT pattern — testing whether training depth or architecture is the primary determinant of trajectory shape.

We extended the OPT analysis with three additional experiments. First, **Gaussian perturbation calibration** on OPT produces near-zero barriers at all ΔW levels (0.0003 at ΔW=0.5% to 0.055 at ΔW=8%), replicating the Pythia finding that unstructured noise creates negligible barriers regardless of magnitude. Second, **training trajectory analysis** on OPT (21 checkpoints across 840 training steps) reveals a monotonically increasing barrier for code (0.14→0.20) and relatively flat barriers for medical (0.09→0.14) — notably different from Pythia's code inverted-U pattern, suggesting that the trajectory shape itself is architecture-dependent. Third, **per-block divergence** on OPT shows a Pearson correlation of r=0.91 between code and medical divergence patterns, somewhat lower than Pythia's r=0.995, consistent with the GPT-2 architecture/tokenizer producing a different layer-wise divergence profile.

We caution that OPT experimental depth is asymmetric (30+ scans vs. 70+ for Pythia), and the experiments use 2-epoch training with 3-point LMC scans (vs. 1-epoch with 11-point for Pythia standard). The trajectory, Gaussian, and per-block results strengthen the cross-architecture evidence but should be interpreted as preliminary rather than definitive.
