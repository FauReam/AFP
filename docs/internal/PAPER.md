...
- **Permutation invariance.** Following Entezari et al. (2022), fine-tuned models from the same pretrained checkpoint are generally permutation-aligned by construction. However, our within-domain barriers (code: 0.048, medical: 0.147) may partially reflect residual permutation drift during aggressive fine-tuning, particularly for medical models. We verify this by running Git Re-Basin weight matching (Ainsworth et al., 2023) on the highest-barrier pair (medical high-divergence s0↔s1, barrier 0.207) and report the barrier before and after alignment in Appendix F.
