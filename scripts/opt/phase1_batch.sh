...
# Usage: nohup bash scripts/phase1_batch.sh >> experiments/phase0_opt/phase1_batch_$(date +%Y%m%d_%H%M%S).log 2>&1 &
# ===========================================================================
set -uo pipefail
...
for domain in math general; do
    cache="data/versaprm/train_${domain}_opt_L256.pt"
    if [ -f "$cache" ]; then
...
tok = AutoTokenizer.from_pretrained('facebook/opt-1.3b', local_files_only=True)
...
pretrained = AutoModel.from_pretrained('facebook/opt-1.3b', local_files_only=True, torch_dtype=torch.bfloat16)
pretrained_sd = {k: v.detach().cpu() for k, v in pretrained.state_dict().items()}
del pretrained; torch.cuda.empty_cache()

torch.manual_seed($s)
random_model = AutoModel.from_pretrained('facebook/opt-1.3b', local_files_only=True, torch_dtype=torch.bfloat16)
