...
V=venv/bin/python3; M=experiments/trained_models; R=experiments/phase0_opt/results
...
orig = torch.load(PROJECT / 'data/versaprm/train_code_opt_L256.pt', map_location='cpu', weights_only=True)
...
val_data = torch.load(PROJECT / 'data/versaprm/val_code_opt_L256.pt', map_location='cpu', weights_only=True)
...
for seed in [10, 11, 12]:
    outdir = PROJECT / f'experiments/trained_models_opt/code_noisy15_lr1e-4_s{seed}'
    ...
    agent = AFPAgent('code', str(device), model_id='facebook/opt-1.3b')
...
for p in "10 11" "10 12" "11 12"; do
    ...
    rm -f "$M/code_e1" "$M/medical_e1" "$R/lmc_barrier_c1m1.json"
    (cd "$M" && ln -sfn _a code_e1 && ln -sfn _b medical_e1)
    $V -u scripts/lmc_3pt_scan.py --model-a "$M/code_noisy15_lr1e-4_s${s1}" --model-b "$M/code_noisy15_lr1e-4_s${s2}" --domain-a code --domain-b code \
...
for name, path, domain in [
    ('code_std', 'experiments/trained_models_opt/code_lr1e-4_s0', 'code'),
    ('med_std', 'experiments/trained_models_opt/medical_lr1e-4_s0', 'medical'),
]:
...
# Hv ≈ (∇L(θ+εv) - ∇L(θ-εv)) / (2ε)
data = load_data('code', 'facebook/opt-1.3b')  # use code data for Hessian
n_samples = 200
...
with open('experiments/phase0_opt/results/hessian_eigenvalues.json', 'w') as f:
