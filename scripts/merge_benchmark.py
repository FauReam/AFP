...
base_model = AutoModel.from_pretrained('EleutherAI/pythia-1.4b', local_files_only=True, torch_dtype=torch.bfloat16)
...
for domain in ['code', 'medical']:
    for s in ['0', '1', '2']:
        path = PROJECT / f'experiments/trained_models/{domain}_lr1e-4_s{s}'
...
# Save
with open('experiments/phase0_ivn/results/merge_benchmark.json', 'w') as f:
