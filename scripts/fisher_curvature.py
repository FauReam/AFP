...
for name, model_path, eval_domain in [
    ...
    agent = AFPAgent(name, str(device), model_id='EleutherAI/pythia-1.4b')
    agent.load(PROJECT / model_path); agent.to_device(); agent.eval_mode()
    data = load_data(eval_domain, 'EleutherAI/pythia-1.4b')
...
with open('experiments/phase0_ivn/results/fisher_curvature.json', 'w') as f:
