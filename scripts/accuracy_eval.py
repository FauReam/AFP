...
agent_c = AFPAgent('code', str(device), model_id='EleutherAI/pythia-1.4b')
agent_m = AFPAgent('medical', str(device), model_id='EleutherAI/pythia-1.4b')
agent_c.load(PROJECT / 'experiments/trained_models/code_lr1e-4_s0')
agent_m.load(PROJECT / 'experiments/trained_models/medical_lr1e-4_s0')
...
sd_c1 = torch.load(PROJECT / 'experiments/trained_models/code_lr1e-4_s1/W_code_final.pt', map_location='cpu', weights_only=True)
pair_results = []
...
sd_m1 = torch.load(PROJECT / 'experiments/trained_models/medical_lr1e-4_s1/W_medical_final.pt', map_location='cpu', weights_only=True)
pair_results = []
for alpha in [0.0, 0.5, 1.0]:
...
with open('experiments/phase0_ivn/results/accuracy_barriers.json', 'w') as f:
