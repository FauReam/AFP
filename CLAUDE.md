...
**每个实验必须回答三个问题，写在 EXPERIMENT_PLAN.md 里：**

1. **为什么跑？** — 假设是什么？
2. **对论文有什么用？** — 高/中/低。能进 figure/table 吗？
3. **是否偏离方向？** — ON TRACK / DRIFTING。

跑完立即判定。偏离方向 = 浪费 GPU 时间。c1m3 和 c1m5 就是例子——13 分钟一个，但只重复了已知结论。

**当前论文主线**：差异度与选择性集成收益的关系。τ=0.5 的门控太弱是直接原因。下一步是测 τ=0.05 和创造对称高差异模型。
